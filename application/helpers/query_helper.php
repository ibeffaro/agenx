...
function get_field_data($field="", $table="",$attr=[],$column=''){
...
function insert_ignore($table="",$data=[]){
    ...
    $db_group->insert_ignore($table,$data);
    $id = $db_group->insert_id();
    if($id)
        return $id;
    else
...
function insert_ignore_batch($table="",$data=[]){
    ...
    return $db_group->insert_ignore_batch($table,$data);
}
...
function get_menu( $table="" , $tbl_menu="", $id = 0 , $parent_id = 0 ){
    $CI         = get_instance();
    $xtable     = explode('.',$table);
    $db_active  = isset($xtable[1]) ? $xtable[0] : 'default';
    $table      = isset($xtable[1]) ? $xtable[1] : $table;
    ...
    $table      = table_prefix($table);
    $tbl_menu   = table_prefix($tbl_menu);

    $query 	    = $db_group->query( 'SELECT id_menu FROM '.$table.' WHERE id_group = '.$id.' AND _view = 1' )->result_array();
    $get_id[] = 0;
    foreach( $query as $q ){
        $get_id[] = $q['id_menu'];
    }
    ...
    return $db_group->get( $tbl_menu )->result_array();
}
...
function get_code($table="",$prefix="",$suffix="",$jumlah_digit=3,$kode="kode") {
    ...
    $db_group->select('MAX(RIGHT(LEFT('.$kode.','.$left.'),'.$jumlah_digit.')) AS k',false);
    $db_group->like($kode,$prefix,'after');
    $db_group->like($kode,$suffix,'before');
    return $db_group->get($table);
}
...
function db_config($param='',$group='') {
    $CI         = get_instance();
    $db_active  = $group ?: 'default';
    $db_group   = $db_active != 'default' ? $CI->load->database($db_active,TRUE) : $CI->db;
    return isset($db_group->$param) ? $db_group->$param : '';
}
...
function id_group_access($target='',$act='view') {
    ...
    $menu       = $db_group->get(table_prefix('menu'))->row_array();
    $id_menu    = isset($menu['id']) ? $menu['id'] : 0;
    $db_group->where('id_menu',$id_menu);
    if(in_array($act,['view','input','edit','delete'])) {
        $db_group->where('_'.$act,1);
    } else {
        $db_group->like('_additional','"'.$act.'":"1"');
    }
    $akses      = $db_group->get(table_prefix('user_akses'))->result();
    $id_group   = [0];
